...
def test_delete_habit(habit_tracker):
    habit_tracker.habit_database.delete_habit = MagicMock()
    habit_tracker.habit_database.get_habits.side_effect = [["Exercise"], []]
    habit_tracker.delete_habit("Exercise")
    assert len(habit_tracker.get_habits()) == 0

def test_check_habit_state_with_habits_tracked(habit_tracker):
...
def test_check_habit_state_when_no_habits_tracked(habit_tracker):
...
def test_view_streaks_with_habits_completed(habit_tracker):
    habit_tracker.habit_database.get_habits = MagicMock(return_value=["Exercise"])
...
def test_view_streaks_when_no_habits_completed(habit_tracker):
